...
function analysisHtml() {
  ...
  if (state.analyzing && !slots.length) return `<div class="analysis-summary"><div><span class="tiny-label">SEGMENTED AI AUDIT</span><h2>Reading the advertorial in smaller batches…</h2><p>This version analyzes each screenshot segment separately, so one long Gemini request cannot take the whole scan down with a 504 timeout.</p></div></div><div class="analysis-loading"><div class="scan-anim">${icon('scan',32)}</div><h3>${esc(state.progress.message || 'Mapping image placements')}</h3><p>Existing images, empty placeholders and new high-value visual opportunities are being audited.</p></div>`;
...
async function analyze() {
  ...
  try {
    ...
    for (let i = 0; i < uploaded.length; i++) {
      ...
      try {
        const part = await apiRetry('analyze-segment', {method:'POST',body:JSON.stringify({
    ...
    try {
      ...
      const fin = await apiRetry('finalize-analysis',{method:'POST',body:JSON.stringify({
    ...
    state.globalError = e.status === 504 ? 'One AI segment timed out. Re-run the scan; V2 will retry each segment separately instead of failing the whole page.' : (e.message || 'Analysis failed.');
  } finally {
